...
import WordDefinition from '../Reader/WordDefinition';
import ReviewCard from '../Reader/ReviewCard';
import Tabs from '../Reader/Tabs';
...
const findEntries = (word) => (word.length === 0 ? null : dictionary[word] || findEntries(word.slice(0, -1)));

const ReadTab = ({ text, selected, words, setText, select, addWord }) => {
	const entries = typeof selected === 'number' ? findEntries(text.slice(selected, selected + 12)) : null;
	...
	return (
		<div>
			<Input
				...
				placeholder="Paste the text that you want to read here"
				aria-label="Save text"
			/>
			<div style={{ backgroundColor: 'white', fontSize: 30, paddingBottom: selected == null ? 40 : 190 }}>
					...
					return (
						<span
							...
							style={{
								...
								backgroundColor: isHighlighted ? '#92CEB5' : ''
							}}
							onClick={() => select(i)}
						>
							{char}
			...
			>
				{entries && entries.length > 0 ? (
					entries.map(([ traditional, simplified, pronunciation, definition ], i) => (
						<WordDefinition
							key={i}
							word={simplified}
	...
	<div>
		{words.length === 0 ? (
			<div
				...
				style={{ height: '90vh', textAlign: 'center', color: 'white' }}
			>
...
const ReviewTab = ({ words, reviewing, backShown, startReviewing, stopReviewing, pass, fail, showBack }) =>
	reviewing ? (
		<div>
			<Button
				variant="contained"
		...
		<div className="d-flex flex-column justify-content-center" style={{ height: '90vh' }}>
			...
			<div style={{ color: '#92CEB5', fontSize: 30, textAlign: 'center' }}>Words to study: {words.length}</div>
			{words.length === 0 ? null : (
				<Button
					variant="contained"
					size="medium"
...
class Reader extends Component {
	...
	render = () => {
		const { text, selected, words, tabIndex, message, reviewing, backShown } = this.state;
		const currentTab = (() => {
			switch (tabIndex) {
				case 0:
					return (
					...
					return (
						<ReviewTab
							...
							pass={() => {
								...
								words[0].nextTime = Date.now() + Math.pow(2, words[0].timeInterval) * 60000;

...
import questions from '../../mockQuestionsThree.json';
import '../../components/quiz/Lesson.css';

const ProgressBar = ({ percent, showQuitModal }) => (
	<div style={{ display: 'flex', alignItems: 'center' }}>
		<div style={{ flex: 1, color: '#ccc', fontSize: 50, cursor: 'pointer' }} onClick={showQuitModal}>
			×
		</div>
		<div
			style={{
...
const Line = () => <div style={{ background: '#ccc', width: '100%', height: 2, margin: '50px 0' }} />;

const SelectedBlocks = ({ blocks, selectedBlockIds, unselectBlock }) => {
	const selectedBlocks = selectedBlockIds.map((blockId) => blocks.find((b) => b.id === blockId));
	return (
		<div
			style={{
				display: 'flex',
		...
		<div>
			...
			>
				...
				>
					...
					<div style={{ textAlign: 'center' }}>All progress in this lesson will be lost.</div>
					<div
						style={{
							display: 'flex',
...
const checkAnswer = ({ questionIndex, correctAnswers, selectedBlockIds }) => {
	const { solutions, blocks } = questions[questionIndex];
	return {
		correctAnswers:
			correctAnswers +
			(solutions.includes(selectedBlockIds.map((id) => blocks.find((b) => b.id === id).text).join('')) ? 1 : 0),
		answered: true
	};
};
...
class LessonThree extends Component {
	...
	render() {
		const { questionIndex, correctAnswers, selectedBlockIds, answered, quitModalShown } = this.state;
		...
		return (
			...
			>
				<ProgressBar percent={correctAnswers / 53 * 100} showQuitModal={() => this.setState(showQuitModal)} />
				<h1 style={{ textAlign: 'center' }}>Translate this sentence</h1>
				<div className="sual" style={{ color: 'white', textAlign: 'center', marginBottom: 20 }}>
